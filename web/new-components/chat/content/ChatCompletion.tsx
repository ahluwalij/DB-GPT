...
const ChatCompletion: React.FC = () => {
  ...
  useAsyncEffect(async () => {
    ...
    if (initMessage && initMessage.id === chatId) {
      ...
      if (res) {
        ...
        const temperature = res?.param_need?.filter(item => item.type === 'temperature')[0]?.value || 0;
        ...
        setTemperatureValue(temperature || 0);
  ...
  return (
    <div className='flex flex-col w-5/6 max-w-4xl mx-auto'>
      {!!showMessages.length &&
