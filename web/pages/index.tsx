...
const Playground: NextPage = () => {
  ...
  function loadMoreRows({ startIndex, stopIndex }: IndexRange) {
    const pageSize = 12;
    const currentPage = Math.ceil(startIndex / pageSize) + 1; // Calculate current page number
    console.log(startIndex, stopIndex, currentPage);
    // This should be an async operation to get more data from server
    // For example, you might call an API and return a Promise
          ...
          {loading && !apps.app_list.length ? (
            ...
            <>
              <InfiniteLoader
                ...
                rowCount={apps.total_count} // Total number of data rows, can be set to a larger number if unknown
              >
